...
---------------------------------------------------
기능적 요구사항
1. <u>고객</u>이 메뉴를 선택하여 **주문**한다.

2. <u>고객</u>이 선택한 메뉴에 대해 **결제**한다.

3. **주문**이 되면 주문 내역이 <u>입점상점주인</u>에게 주문정보가 전달된다

4. <u>상점주</u>는 주문을 **수락**하거나 **거절**할 수 있다. 

5. (신규) **수락**의 경우 고객의 지역 인근의 <u>라이더</u>들에게 **배송(=수락) 건을 조회**할 수 있다. 

6. (신규) **거절**의 경우에는 **결제취소 처리**한다.

7. (신규) <u>상점주</u>는 음식메뉴를 추가/변경/삭제/판매중지 할 수 있다. 메뉴추가 시 **카톡 알림**을 보낸다.

8. (신규)주문은 별도로 선호도 관리를 하여 향후 마케팅 자료로 사용한다. 매일12시에는 추천매뉴를 카톡알림으로 보낸다.

9. <u>상점주</u>는 **요리**시작때와 완료 시점에 시스템에 **상태를 입력**한다

10. <u>고객</u>은 아직 **요리**가 시작되지 않은 **주문은 취소**할 수 있다

11. **요리가 완료**되면 **&주문상태가 변경되고** 고객의 지역 인근의 <u>라이더</u>들에 의해 **배송건 조회가 가능**하다

12. <u>라이더</u>가 해당 요리를 pick 한후, pick했다고 **앱을 통해 통보**한다.

13. <u>고객</u>이 **주문상태**를 중간중간 조회한다

14.  **주문상태**가 바뀔 때 **마다 카톡으로 알림**을 보낸다

15.  <u>고객</u>이 요리를 배달 받으면 **배송확인 버튼**을 탭하여, 모든 **거래가 완료**된다
